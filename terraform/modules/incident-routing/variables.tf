...
variable "require_pager" {
  description = "When true, planning fails if pager_endpoint is null. Set this in prod. Enforced by a precondition on the pager DLQ (a check block would only warn)."
  type        = bool
  default     = false
}
